...
const BottomSheetComponent = () => {
  ...
  const snapPoints = useMemo(() => ["17%", "43%"], []);
  ...
  const handleSelectPhotoFromAlbum = useCallback(
    (uri: string) => {
      setPhoto(uri);
      router.push("/camera");
    },
        ...
        <BottomSheetView style={styles.content}>
          <View style={styles.buttonsContainer}>
            ...
            <CustomPressable
              ...
              style={[styles.button, { backgroundColor: galleryButton }]}
            >
              <Ionicons name="images-outline" size={24} color="#FFFFFF" />
              <Typography type="subtitle" lightColor="#FFFFFF">
                {t("gallery_title")}
              </Typography>
            </CustomPressable>
            {/* camera button */}
            <CustomPressable
              ...
              style={[styles.button, { backgroundColor: cameraButton }]}
            >
          ...
          {albums && albums.length > 0 && (
            <View style={styles.albumsContainer}>
              <Typography type="title" lightColor="#FFFFFF">
                {t("albums_title")}
              </Typography>
              <FlatList
                data={albums}
                ...
                renderItem={({ item }) => (
                  <CustomPressable
                    onPress={() => handleSelectPhotoFromAlbum(item.uri)}
                  >
                    <Image
                      source={{ uri: item.uri }}
